...
describe("coop.examples", function()
	...
	it("run_parallel_search works", function()
		local fast_result, slow_result = coop.spawn(examples.run_parallel_search):await(100, 20)
		assert.are.same("fast", fast_result)
		assert.are.same("cancelled", slow_result)
	end)
end)
